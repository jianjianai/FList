...
const children = computed(()=>{
  if(sortType.value==="name-asc"){
    return [...frontmatter.value.folder?.children || []].sort((a,b)=>{
  ...
  return frontmatter.value.folder?.children
});
    ...
    <template #default v-if="children && children.length>0">
      <div class="files">
        ...
        <TransitionGroup name="list">
          <FButtonLink class="td" v-for="file of children" :key="file.name" :to="`./${encodeURI(file.name)}/`">
            <div class="t-name">
